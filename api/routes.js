...
export async function fetchCentrais() {
  ...
  const resp = await fetch('https://youdo.cloudns.nz/centrais', {
    method: 'GET',
    headers: {
      'token': token,
...
export async function fetchEquipamentos(device_id = null) {
  ...
  let url = 'https://youdo.cloudns.nz/equipamentosdb';
...
export async function fetchClientes(equipamentoID = null) { // Torne o parâmetro opcional novamente
  ...
  if (equipamentoID) {
    ...
    url = `https://youdo.cloudns.nz/usuarios/central?equipamento=${equipamentoID}`;
  } else {
    // Se nenhum equipamentoID for fornecido, chamamos a rota para LISTAR TODOS OS USUÁRIOS
    url = 'https://youdo.cloudns.nz/usuarios'; // Assumindo que esta rota lista todos
  }
...
export async function fetchClientesPorCentral(centralId) {
  ...
  const url = `https://youdo.cloudns.nz/usuarioslocal?central=${centralId}`;
...
export async function fetchClientePorIdYD(idyd) {
  ...
  const url = `https://youdo.cloudns.nz/usuarios?idyd=${idyd}`;
